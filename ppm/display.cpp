  ...
  while(iterations-->0) {
    ...
    for (uint16_t y = 0; y < 320 ; y++) {
      for (uint16_t x = 0; x < 480 ; x++) {
        //Color color(50,40,10);
        //color = Color(255,255,255);
        //color = reader.nextColor();
        //if(sqrt((double)((circle_mid_x - x) * (circle_mid_x - x) + (circle_mid_y-y) *(circle_mid_y-y)))<circle_r) {
        //    color.b = 255;
        //}
        //if(iterations == 0) 
        //    parlcd_write_data(parlcd_mem_base, Color(0,0,0));
        //else 
            parlcd_write_data(parlcd_mem_base, reader.nextColor());
      }
    }
    //break;
    circle_mid_x++; circle_mid_y++;
  }
/*
  parlcd_write_cmd(parlcd_mem_base, 0x2c);
  for (i = 0; i < 320 ; i++) {
...
/*
  ...
  while (max-->0) {
     ...
     *(volatile uint16_t*)(parlcd_mem_base + PARLCD_REG_DATA_o) = 0x0000;
     *(volatile uint16_t*)(parlcd_mem_base + PARLCD_REG_DATA_o) = 0x0000;
     *(volatile uint16_t*)(parlcd_mem_base + PARLCD_REG_DATA_o) = 0x0000;
     *(volatile uint16_t*)(parlcd_mem_base + PARLCD_REG_DATA_o) = 0x0000;
     *(volatile uint32_t*)(parlcd_mem_base + PARLCD_REG_DATA_o) = 0x0000;
     *(volatile uint16_t*)(parlcd_mem_base + PARLCD_REG_DATA_o) = 0x1000;
